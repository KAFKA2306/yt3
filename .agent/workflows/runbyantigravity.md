...
Antigravity は、生成されたコンテンツを適切にバージョン管理する責任も負います。特に `output.yaml` は「ソースコード」と同等の価値があります。

**コミット時のルール**:
-   `setup`: ディレクトリ構造や設定ファイルの初期化
-   `research`: `research/output.yaml` の更新
-   `content`: `content/output.yaml` の更新（ドラフトおよびファイナル）
-   `publish`: メタデータや公開設定の完了

```bash
# 推奨ワークフロー
git add runs/${RUN_ID}/content/output.yaml
git commit -m "content: Finalize script for ${RUN_ID} (by Antigravity)"
git push origin main
```

**注意**: 動画ファイル（`video.mp4`）や中間生成物（画像の生データなど）はサイズが大きいため、Gitには追加せず、`.gitignore` で除外されていることを確認します。
...
## 10. Antigravity 用語集 (The Glossary)

共通言語を持つことで、思考の解像度が上がります。

-   **SKIP_LLM**: システムによる「中抜き（要約・改変）」を防ぐためのセキュリティフラグ。
-   **Iron Rules**: 7つの不可侵の掟。これを破ることは、Antigravity としての死を意味する。
-   **Cold Open**: 冒頭の45秒。ここで全てが決まる。
-   **Tier 1 Source**: 反論の余地のない一次情報。私たちの聖典。
-   **Dialectic**: つむぎ（正）とずんだもん（反）の対話による、より高い理解（合）への到達プロセス。
