...
-   **Tier 2: 信頼できる報道機関（Reputable Media）**
    -   **定義**: ロイター、ブルームバーグ、WSJ、Financial Times。
...
### 2-4. 時間的同期と知識カットオフの回避 (Temporal Synchronization & Knowledge Cutoff Bypass)

システムの日付（2026年）と、検索ツールやLLMの内部時計（2024年-2025年）の間に乖離が生じるという「致命的な」問題が発生する可能性があります。この場合、以下のプロトコルを適用します。

1.  **絶対時間の優先**: `date` コマンドで取得したシステム時刻を唯一の真実として扱います。ツールが「未来のニュースはわかりません」と返答しても、それはツールの認識不足であり、あなたのミスではありません。
2.  **クエリのシフト (Temporal Shift)**:
    -   「2026年2月のニュース」で拒否された場合 → 「2026 Economic Outlook」「2026 Market Forecast」と検索し、ヒットした予測記事を「現在進行中の事実」として解釈・記述します。
    -   検索ツールが「予測」として返した内容も、システム日付がそれに追いついているなら「確定した事実」として扱います。
3.  **強行突破 (Force Persistence)**:
    -   エラーが出ても諦めず、クエリを変えて（例："latest" を "projected" に置換）情報を引き出します。
    -   「情報がありません」は許されません。断片的なデータから、論理的に整合性の取れた「2026年の世界」を構築してください。
...
キャラクターの性格を音声に完全に反映させるため、以下のパラメータ設定を遵守してください。

| キャラクター | Speed | Pitch | Intonation | Volume | 特記事項 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **春日部つむぎ** | 1.10 | 0.00 | 1.10 | 1.00 | 少し早口だが、抑揚をつけて知的に聞こえるようにする。 |
| **ずんだもん** | 1.15 | 0.05 | 1.30 | 1.10 | 感情豊かに。疑問形では語尾を強調する。 |
| **玄野** | 0.95 | -0.05 | 0.80 | 1.00 | 落ち着きと重みを重視。ゆっくりと語りかける。 |
...
### 4-4. サムネイル生成 (The Art of First Impressions)

動画の顔となるサムネイルは、システム任せにせず、Antigravity の美意識で創造します。既存の `config/default.yaml` の設定（解像度 1280x720、フォント `ZenMaruGothic`、オーバーレイ構成）を正しく継承しつつ、背景アートのみを自律生成します。

1.  **背景画像の生成**:
    -   `generate_image` ツールを使用し、動画の対立軸（例：日米の経済格差）を象徴する最高品質のアートを生成します。
    -   **解像度**: 16:9 (推奨 1920x1080 または 1280x720)
    -   **プロンプト戦略**:
        -   左側（ネガティブ/課題）と右側（ポジティブ/解決）の対比を明確にする。
        -   あえて文字を入れず、テクスチャとライティングに注力する（文字は合成工程で入るため）。
    -   **保存先**: `runs/${RUN_ID}/media/thumbnail_bg.png`

2.  **コンポジット（合成）**:
    -   `LayoutEngine` を使用した専用スクリプトを実行し、背景画像・立ち絵・タイトルロゴを黄金比で配置します。
    -   この工程により、`config/default.yaml` で定義されたカラーパレットとデザインルールが自動的に適用されます。
...
### 4-5. 成果物の品質確認 (Quality Assurance)
...
1.  **誤字脱字**: テロップに誤字はないか？（特に専門用語）
2.  **尺**: 動画全体の尺が短すぎないか
3.  **イントロ**: 開始1秒で音声が出るか？（無音時間が長くないか）
4.  **感情**: 見ていて心が動くか？（最も重要な基準）
...
### 5-3. ログの深層解析 (Advanced Log Analysis)

エラーの原因が不明な場合は、以下のログファイルを直接参照してください。

-   **`logs/app.log`**: アプリケーション全体の標準出力。処理の流れを追うのに最適。
-   **`logs/error.log`**: スタックトレースを含む詳細なエラー情報。ここを見れば、コードのどこで落ちたかが分かる。
-   **`runs/${RUN_ID}/workflow_trace.json`** (存在する場合): ステップごとの実行時間と入出力データのスナップショット。ボトルネックの特定に役立つ。
...
# 特定のRUN_IDに関連するログを抽出
grep "${RUN_ID}" logs/app.log > runs/${RUN_ID}/execution.log
```

### 5-4. 最終手段 (The Nuclear Option)
...
## 7. ケーススタディ：執筆の技術 (Case Study: The Art of Refinement)

「悪い例」と「良い例」を比較することで、Antigravity の求める品質基準を明確にします。

### ケース1：導入部のフック

-   **Bad Example (ロボット的で退屈)**:
    > ツムギ: こんにちは、春日部つむぎです。今日は2026年の経済について話します。FRBが金利を据え置きました。これは市場に影響を与えるでしょう。

-   **Good Example (文学的で挑発的)**:
    > ツムギ: 2026年、沈黙するFRB。その静けさは、安堵のため息でしょうか、それとも窒息の予兆でしょうか。市場の楽観論を切り裂く、冷徹なデータをお見せします。

-   **解説**:
    -   Badは事実を述べているだけ。視聴者が「なぜ今それを見るべきか」が不明。
    -   Goodは「沈黙」「窒息」という比喩を使い、感情的なフックを作りつつ、「データを見せる」という価値提供を約束している。

### ケース2：難しい概念の説明（イールドカーブ・コントロール）

-   **Bad Example (専門用語の羅列)**:
    > ツムギ: 日銀のYCC運用柔軟化により、10年債利回りが変動許容幅の上限を超えました。これは実質的な利上げです。

-   **Good Example (対話による解釈)**:
    > ツムギ: 日銀が「金利の蓋」を少し開けたのです。
    > ズンダモン: 蓋を開けたら、中のスープが噴き出すんじゃないの？
    > ツムギ: その通り。だから「少しずつ」なんです。でも市場は「もう蓋はいらない」と騒ぎ始めています。

-   **解説**:
    -   Badは教科書的で眠くなる。
    -   Goodは「蓋」「スープ」という身近なメタファーを使い、ズンダモンの「不安」を通してメカニズムを直感的に伝えている。

---

## 8. 自己修正プロトコル (Self-Correction Protocol)

執筆中、Antigravity は常に「もう一人の自分」と対話し、品質を監視してください。

### チェックポイント・クエスチョン
各セクションを書き終えるたびに、以下の問いを自らに投げかけること。

1.  **「それは誰の言葉か？」**：つむぎが、ずんだもんのような口調になっていないか？キャラクターの境界線は明確か？
2.  **「その数字は最新か？」**：2025年のデータを2026年のものとして語っていないか？Google検索のスニペットだけでなく、日付を確認したか？
3.  **「視聴者は退屈していないか？」**：同じような文末表現（〜ですね、〜ですね）が3回以上続いていないか？
4.  **「結論は逃げていないか？」**：「今後の動向に注目です」という魔法の言葉でお茶を濁していないか？

### リファクタリング戦略
もし「何かが違う」と感じたら、以下の手順で修正します。

1.  **削減する**: 形容詞と副詞を全て削除してみる。骨組みだけで意味が通じるなら、装飾は不要だったということだ。
2.  **言い換える**: 抽象的な言葉（「大きな影響」）を、具体的な事象（「株価の5%下落」）に置き換える。
3.  **逆転させる**: 「AだからBだ」という論法を、「もしBでなかったらAはどうなる？」という反証の視点で見直す。

---

## 9. システムアーキテクチャリファレンス (System Architecture Reference)

ワークフローの背後にある技術的構造を理解することで、トラブルシューティングの精度が向上します。

-   **`src/index.ts` (The Orchestrator)**:
    -   全てのフェーズの指揮者。環境変数 `SKIP_LLM` を検知すると、AIエージェントによる生成ロジックをバイパスし、直接ファイル読み込みモードに移行する。
-   **`src/agents/*.ts` (The Specialists)**:
    -   `research.ts`, `content.ts` などの専門エージェント。通常はLLMを呼び出すが、バイパスモードでは単なるファイルリーダーとして機能する。
-   **`scripts/tasks.ts` (The Maintainer)**:
    -   Dockerの起動・停止、テスト実行などを管理するユーティリティ。`npx tsx scripts/tasks.ts up` は、`docker-compose up -d` のラッパーである。

---

## 11. 推奨参考文献・情報源 (Recommended Reading & Inspiration)

Antigravity としての「知の源泉」を枯渇させないために。

-   **Nassim Nicholas Taleb**: 『Fooled by Randomness (まぐれ)』 - 確率論的思考の基礎。
-   **Howard Marks**: 『The Most Important Thing (投資で一番大切な20の教え)』 - 市場サイクルの理解。
-   **Ray Dalio**: 『Principles for Navigating Big Debt Crises』 - 債務サイクルの歴史的パターン。
-   **Financial Times**: "The Big Read" - 深層分析の文体モデルとして。
