...
async function main() {
	...
	if (!tasksPath) {
		Logger.error(
			...
			"EXPANSION",
			"INIT_FAIL",
			"expansion_tasks.json not found in any dated runs directory",
		);
		return;
	}
	...
	for (const task of taskList.tasks) {
		if (
			...
			task.current_chars >= taskList.target_chars
		)
			continue;
		...
		Logger.info(
			...
			"EXPANSION",
			"START",
			`Expanding ${task.project} (${task.current_chars} chars)`,
		);

		const systemPrompt =
			"あなたはASMR台本の執筆者です。指定された文字数（最低5000文字）を確実に超えるように拡張してください。曖昧な指示ではなく、物理的な文字数を稼ぐために詳細な情景描写と内面描写を執筆してください。絶対にファイル名、パス、技術的な指示（例: ffmpeg）などのメタデータを出力に含めないでください。";
		...
		try {
			...
			if (!validationResult.success) {
				Logger.error(
					...
					"EXPANSION",
					"VALIDATION_FAIL",
					`Validation failed for ${task.project}: ${validationResult.error}`,
				);
			...
			Logger.info(
				...
				"EXPANSION",
				"RESULT",
				`${task.project} -> ${newCharCount} chars`,
			);

			await fs.writeFile(scriptPath, expandedScript);
			task.current_chars = newCharCount;
			task.status =
				newCharCount >= taskList.target_chars ? "completed" : "pending_retry";
		} catch (err) {
			Logger.error(
				"HARNESS",
				"EXPANSION",
				"TASK_FAIL",
				`Failed to expand ${task.project}: ${err}`,
			);
