...
async function runStep(
  ...
  const agents: Record<string, () => Promise<Partial<AgentState>>> = {
    ...
    "content:fix": async () => {
      ...
      const res = await new ScriptSmith(store).run(
        state.news || [],
        state.director_data,
        state.memory_context || "",
      );
    ...
    evaluate: async () => {
      ...
      return { evaluation: await new CriticAgent(store).run(state.script) };
    },
    media: async () => {
      if (!state.script) throw new Error("script is missing");
    ...
    all: async () => {
      await createGraph(store).invoke({
        run_id: store.runDir.split("/").pop()!,
        bucket: state.bucket || bucket,
      });
      await sendAlert(`Workflow finished: ${state.bucket || "unknown"}`, "success");
      return {};
    },
  };
