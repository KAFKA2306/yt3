...
export class ThumbnailRenderer {
    ...
    selectBestPalette(palettes: Palette[]): Palette {
        ...
        interface ScoredPalette { palette: Palette; score: number; }
    ...
    async render(plan: RenderPlan, title: string, output: string): Promise<void> {
        ...
        layers.push({ input: Buffer.from(this.createSvg(title, textMaxX, cfg, palette)), top: 0, left: 0 });
    ...
    private createSvg(title: string, maxX: number, cfg: AppConfig["steps"]["thumbnail"], pal: Palette): string {
        ...
        const txt = lines.map((l, i) => {
            const y = startY + i * lh;
            const escaped = l.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').toUpperCase();
