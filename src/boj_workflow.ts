...
async function main() {
	...
	AgentLogger.info(
		...
		"BOJ_WORKFLOW",
		"START",
		"Starting Autonomous BoJ Revision Workflow",
	);
	...
	const title =
		"日本銀行：需給ギャップ・潜在成長率の推計方法見直しと労働需給指標の活用 (2026)";
	const bojContent = `
...
こうした点を踏まえ、賃金・物価の予測力という観点からみて、需給ギャップと補完的にモニタリングしていくことが適当とみられる労働需給関連指標についても、需給ギャップ・潜在成長率と合わせ、四半期に一度、「需給ギャップ・潜在成長率および労働需給関連指標」のページ内で公表を行うこととします。
	...
	try {
		...
		await nlmAgent.addSource(
			...
			bojContent,
			"text",
			"BoJ Official Announcement",
		);

		const deepQuery =
			"Bank of Japan output gap potential growth rate revision March 2026 labor supply demand indicators";
		...
		const state: AgentState = {
			...
			metadata: {
				title: title,
				description:
					"日本銀行（日銀）が2026年3月に発表した「需給ギャップと潜在成長率」の推計方法見直しについて、NotebookLMのAI解説をお届けします。労働需給指標の補完的活用など、今後の金融政策判断における重要ポイントを深掘りします。",
		...
		AgentLogger.info(
			...
			"BOJ_WORKFLOW",
			"SUCCESS",
			"Autonomous BoJ workflow completed!",
			{
