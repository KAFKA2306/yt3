...
export class PublishAgent extends BaseAgent {
  ...
  async run(state: AgentState): Promise<PublishResults> {
    ...
    if (ytStep?.enabled) results.youtube = await this.uploadToYouTube(state, this.config);
    const twStep = this.config.steps.twitter;
    if (twStep?.enabled) results.twitter = await this.postToTwitter(state, this.config);
