...
import { AssetStore, BaseAgent, ROOT, getRunIdDateString } from "../core.js";
...
export class ManagerAgent extends BaseAgent {
	...
	constructor(store: AssetStore, opts: Record<string, unknown> = {}) {
		...
		this.mConfig = {
			check_interval_ms:
				(opts.check_interval_ms as number) || mCfg.check_interval_ms,
			stale_lock_timeout_seconds:
				(opts.stale_lock_timeout_seconds as number) ||
				mCfg.stale_lock_timeout_seconds,
	...
	async run() {
		AgentLogger.info(
			...
			"STARTUP",
			"INIT",
			"AI Manager Agent starting persistent loop...",
		);
		await this.repairSystem();
		while (true) {
			await this.orchestrationCycle();
			await new Promise((resolve) =>
				setTimeout(resolve, this.mConfig.check_interval_ms),
			);
		}
	}
	private async orchestrationCycle() {
		AgentLogger.info(
			...
			"ORCHESTRATION",
			"CHECK",
			"Starting periodic health and run check",
		);
	...
	private async repairSystem() {
		AgentLogger.info(
			...
			"REPAIR",
			"START",
			"Running system self-repair checks",
		);
		const lingerCheck = spawnSync("loginctl", [
			"show-user",
			this.mConfig.user,
			"--property=Linger",
		]);
		if (!lingerCheck.stdout.toString().includes("Linger=yes")) {
			AgentLogger.warn(
				...
				"REPAIR",
				"LINGER_FIX",
				"Linger disabled. Attempting to enable...",
			);
			...
			AgentLogger.info(
				...
				"REPAIR",
				"LINGER_FIXED",
				`Linger enabled for ${this.mConfig.user} user`,
			);
		}
	}
		...
		if (fs.existsSync(lockFile)) {
			...
			if (ageSeconds > this.mConfig.stale_lock_timeout_seconds) {
				AgentLogger.warn(
					...
					"REPAIR",
					"LOCK_STALE",
					`Found stale lock file (age: ${Math.round(ageSeconds)}s). Removing...`,
					{
	...
	private async checkDailyRun() {
		const today = getRunIdDateString();
		...
		if (!fs.existsSync(runDir)) {
			...
			if (hour >= this.mConfig.start_hour) {
				AgentLogger.decision(
					...
					"ORCHESTRATION",
					"TRIGGER_RUN",
					`No run found for today (${today}). Triggering catch-up run.`,
					{
