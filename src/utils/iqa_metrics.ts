...
export async function calculateMobileEdgeStrength(
	imagePath: string,
): Promise<number> {
	const { data, info } = await sharp(imagePath)
		.resize(150)
	...
	for (let y = 1; y < height - 1; y++) {
		for (let x = 1; x < width - 1; x++) {
			const gx =
				(px[y * width + (x + 1)] ?? 0) - (px[y * width + (x - 1)] ?? 0);
			const gy =
				(px[(y + 1) * width + x] ?? 0) - (px[(y - 1) * width + x] ?? 0);
...
export async function calculateXHeightScore(
	imagePath: string,
): Promise<number> {
	const { data, info } = await sharp(imagePath)
		.grayscale()
