...
export class LayoutEngine {
  ...
  constructor() {
    ...
    const vResParts = (videoCfg.resolution || "1920x1080").split("x").map(Number);
    this.videoRes = { width: vResParts[0] ?? 1920, height: vResParts[1] ?? 1080 };
    const tResParts = (thumbCfg.resolution || "1280x720").split("x").map(Number);
    this.thumbRes = { width: tResParts[0] ?? 1280, height: tResParts[1] ?? 720 };
  }
  ...
  private generateASSDialogLines(
    ...
    for (let i = 0; i < script.lines.length; i++) {
      ...
      const { formattedText: txt, fontSize: fz } = fitText(line.text, baseFz, maxW, minFz);
