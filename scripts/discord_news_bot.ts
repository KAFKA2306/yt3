...
async function main() {
  ...
  const strTemplate = settings.starter_template || "Starting generation for: {query}";
  const resTemplate = settings.response_template || "Generating video for: {query}";
  ...
  const commands = [
    new SlashCommandBuilder()
      ...
      .addStringOption((option) =>
        option.setName("query").setDescription("Topic to generate news about").setRequired(true),
      ),
  ];
  ...
  client.on("interactionCreate", async (interaction) => {
    ...
    if (interaction.commandName === commandName) {
      ...
      const starterMsg = await channel.send(strTemplate.replace("{query}", query));
