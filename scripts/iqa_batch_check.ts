...
import { type BackgroundRisk, IQA_THRESHOLDS, IqaValidator } from "../src/utils/iqa_validator.js";
...
async function auditImage(imagePath: string): Promise<BatchResult> {
  ...
  if (result.textLayout?.isTextClipped)
    failReasons.push(`TEXT_CLIPPED: ${(result.textLayout.clipBoundaryRatio * 100).toFixed(1)}%`);
...
function auditPalettes(): void {
  ...
  const entries = palettes
    .map((p, i) => {
      ...
      const mobilePred =
        risk === "low" ? "≥ 35 (安全)" : risk === "medium" ? "~25-35 (限界)" : "< 25 (危険)";
    ...
    .sort((a, b) => {
      ...
      return (
        (order[a.rating as keyof typeof order] ?? 3) - (order[b.rating as keyof typeof order] ?? 3)
      );
    });

  for (const { i, p, contrast, risk, mobilePred, rating } of entries) {
    const riskColor =
      risk === "low" ? COLORS.green : risk === "medium" ? COLORS.yellow : COLORS.red;
    const wcag =
      contrast >= 7.0
