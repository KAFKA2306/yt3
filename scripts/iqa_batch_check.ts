...
async function auditImage(imagePath: string): Promise<BatchResult> {
    ...
    if (!fs.existsSync(imagePath) || fs.statSync(imagePath).size === 0) {
        return { imagePath, runId, passed: false, score: 0, sharpness: 0, contrastRatio: 0, mobileEdgeStrength: 0, isResolutionCorrect: false, backgroundRisk: 'low', failReasons: ['FILE_NOT_FOUND_OR_EMPTY'] };
    }

    const result = await validator.validate(imagePath, textHex, bgHex, undefined, guardBand);
    const m = result.metrics;

    if (!m.isResolutionCorrect) failReasons.push(`RESOLUTION_MISMATCH`);
    if (m.sharpness < IQA_THRESHOLDS.SHARPNESS_MIN) failReasons.push(`SHARPNESS_LOW: ${m.sharpness.toFixed(2)}`);
    if (m.contrastRatio < IQA_THRESHOLDS.CONTRAST_MIN) failReasons.push(`CONTRAST_LOW: ${m.contrastRatio.toFixed(2)}`);
    if ((m.mobileEdgeStrength ?? 0) < IQA_THRESHOLDS.MOBILE_EDGE_MIN) failReasons.push(`MOBILE_EDGE_WEAK: ${m.mobileEdgeStrength?.toFixed(2)}`);
    if (result.textLayout?.isTextClipped) failReasons.push(`TEXT_CLIPPED: ${(result.textLayout.clipBoundaryRatio * 100).toFixed(1)}%`);
    if (result.textLayout?.isTextOverlappingCharacter) failReasons.push(`TEXT_OVERLAPS_CHARACTER: ${(result.textLayout.overlapRatio * 100).toFixed(1)}%`);

    const edgeStrength = m.mobileEdgeStrength ?? 0;
    const score = (m.isResolutionCorrect ? 0.1 : 0)
    ...
    return {
        imagePath, runId, passed: failReasons.length === 0, score,
        sharpness: m.sharpness, contrastRatio: m.contrastRatio,
        mobileEdgeStrength: edgeStrength, isResolutionCorrect: m.isResolutionCorrect,
...
function printResult(r: BatchResult, index: number, total: number): void {
    ...
    const riskColor = r.backgroundRisk === 'low' ? COLORS.green : r.backgroundRisk === 'medium' ? COLORS.yellow : COLORS.red;
    ...
    r.failReasons.forEach(reason => console.log(`  ${COLORS.yellow}⚠ ${reason}${COLORS.reset}`));
}
...
function printSummary(results: BatchResult[]): void {
    ...
    console.log(`  総数: ${results.length}  合格: ${COLORS.green}${passed}${COLORS.reset}  不合格: ${COLORS.red}${failed}${COLORS.reset}  合格率: ${((passed / results.length) * 100).toFixed(1)}%`);
    ...
    top5.forEach((r, i) => console.log(`  ${i + 1}. ${(r.score * 100).toFixed(1)}%  ${r.runId}  (sharp: ${r.sharpness.toFixed(0)}, mobile: ${r.mobileEdgeStrength.toFixed(1)})`));
}

function auditPalettes(): void {
    const palettes = thumbCfg.palettes;
    const ratingOf = (contrast: number, risk: BackgroundRisk) =>
        (contrast >= IQA_THRESHOLDS.CONTRAST_GOAL && risk === 'low') ? '✅ BEST'
            : (contrast >= 4.5 && risk !== 'high') ? '⚠ OK' : '❌ RISKY';

    console.log(`\n${COLORS.bold}${COLORS.cyan}パレット監査レポート${COLORS.reset}`);
    console.log('═'.repeat(76));
    console.log(`  #  背景色       テキスト色    コントラスト  WCAG  リスク    MOBILE予測         評価`);
    console.log('─'.repeat(76));

    const entries = palettes.map((p, i) => {
        const contrast = validator.calculateContrastRatio(p.title_color, p.background_color);
        const risk = validator.analyzeBackgroundRisk(p.background_color);
        const mobilePred = risk === 'low' ? '≥ 35 (安全)' : risk === 'medium' ? '~25-35 (限界)' : '< 25 (危険)';
        const rating = ratingOf(contrast, risk);
        return { i, p, contrast, risk, mobilePred, rating };
    }).sort((a, b) => {
        const order = { '✅ BEST': 0, '⚠ OK': 1, '❌ RISKY': 2 };
        return (order[a.rating as keyof typeof order] ?? 3) - (order[b.rating as keyof typeof order] ?? 3);
    });

    for (const { i, p, contrast, risk, mobilePred, rating } of entries) {
        const riskColor = risk === 'low' ? COLORS.green : risk === 'medium' ? COLORS.yellow : COLORS.red;
        const wcag = contrast >= 7.0 ? `${COLORS.green}AAA✓${COLORS.reset}` : contrast >= 4.5 ? `${COLORS.yellow}AA✓${COLORS.reset}` : `${COLORS.red}FAIL${COLORS.reset}`;
        console.log(
    ...
    console.log('─'.repeat(76));
    console.log(`  推奨: ${entries.filter(e => e.rating === '✅ BEST').length} パレット  要注意: ${entries.filter(e => e.rating === '❌ RISKY').length} パレット`);
}
...
async function main(): Promise<void> {
    ...
    const imagePaths = (await glob(pattern, { cwd: process.cwd() })).map(p => path.join(process.cwd(), p));

    console.log(`${COLORS.bold}${COLORS.cyan}IQA バッチ審査  — 対象: ${imagePaths.length} 枚${COLORS.reset}\n${'─'.repeat(70)}`);
    ...
    const logPath = path.join(process.cwd(), 'logs', 'visual_quality_audit.json');
    ...
    await fs.writeJson(logPath, {
        ...
        results: results.map(r => ({ ...r, imagePath: r.imagePath.replace(process.cwd() + '/', '') })),
    }, { spaces: 2 });
