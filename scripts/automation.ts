...
async function startServices(automation: any) {
    ...
    for (const service of (automation.services || [])) {
        ...
        if (service.background !== false) {
            const child = spawn(cmdList[0], cmdList.slice(1), { cwd, env: { ...process.env, ...(service.env || {}) }, stdio: ['ignore', out, out], detached: true });
            child.unref();
        } else {
            spawnSync(cmdList[0], cmdList.slice(1), { cwd, env: { ...process.env, ...(service.env || {}) }, stdio: ['ignore', out, out] });
        }
    }
}
...
function buildSchedule(automation: any): string[] {
    ...
    for (const schedule of (automation.schedules || [])) {
        ...
        fs.ensureDirSync(path.dirname(lp));
        const envExports = Object.entries(schedule.env || {}).map(([k, v]) => `${k}='${v}'`).join(' ');
        lines.push(`${schedule.cron} cd ${cwd} && ${envExports ? envExports + ' ' : ''}${[...(schedule.command || []), ...(schedule.args || [])].join(' ')} >> ${lp} 2>&1`);
    }
    return lines;
}
